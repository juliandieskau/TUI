...
void SystemMontitoringPlugin::subcribeToROS(){ 
  ...
  rosbridge_client_cpp::Subscriber my_sub6(*ros, "/ects/system/disk/{mountpoint}/usage", "DiskUsage.msg", my_callback6, 5);
  ...
  rosbridge_client_cpp::Subscriber my_sub9(*ros, "/ects/system/network/{adapter}/info", "NetworkInfo.msg", my_callback9, 5);
  
  auto my_callback10 = [&](const picojson::object& json){ networkusage = std::to_string(json); };
  rosbridge_client_cpp::Subscriber my_sub10(*ros, "/ects/system/network/{adapter}/usage", "NetworkUsage.msg", my_callback10, 5);
  
  auto my_callback11 = [&](const picojson::object& json){ memoryUsagehist = std::to_string(json); };
  rosbridge_client_cpp::Subscriber my_sub11(*ros, "/ects/system/averages/{aggregation_name}/mem/usage", "MemoryUsageHistory.msg", my_callback11, 5);
